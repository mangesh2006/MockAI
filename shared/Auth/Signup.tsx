...
const Signup = () => {
  ...
  const handleSignup = async (data: UserSchema) => {
    try {
      const api = await fetch("/api/v1/auth/signup", {
        method: "POST",
  ...
  return (
    <Dialog>
      <DialogTrigger className={"cursor-pointer bg-blue-500 p-2 h-full font-semibold text-white tracking-wider transition duration-400 hover:bg-blue-600 rounded-lg"}>
        Signup
      </DialogTrigger>

      <DialogContent className={"bg-white text-black"}>
        <DialogHeader>
          <DialogTitle className={"text-2xl font-bold"}>Signup</DialogTitle>
        </DialogHeader>

        <form onSubmit={handleSubmit(handleSignup)} className='p-2 flex flex-col w-full gap-4'>
          <div className='flex flex-col gap-2'>
            <label htmlFor="name" className='font-semibold'>Enter your name</label>
            <input type="text" className='p-2 rounded-lg border' placeholder='Enter your name' {...register("name")} />
          </div>
          ...
          <div className='flex flex-col gap-2'>
            <label htmlFor="password" className='font-semibold'>Create your password</label>
            <input type="password" className='p-2 rounded-lg border' placeholder='Enter your password' {...register("password")} />
          </div>
          {errors.password && <p className="text-red-500">{errors.password.message}</p>}

          <button type="submit" className="cursor-pointer bg-blue-500 p-2 h-full font-semibold text-white tracking-wider transition duration-400 hover:bg-blue-600 rounded-lg w-full flex items-center justify-center" disabled={isSubmitting}>
